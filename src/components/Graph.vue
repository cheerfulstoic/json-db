...
export default defineComponent({
  name: 'Graph',
  props: {
    database: {type: db.Database, required: true},
    current_focus: { type: Object, required: false},
  },
  mounted() {
    this.refresh()
  },
  ...
  methods: {
    default_selected_references() {
      return _.reduce(
        this.database.sheets,
        (result, sheet) => {
          return _.reduce(
            this.references_for(sheet),
            (sub_result: any, definition) => {
              if (sub_result[sheet._id] == null) {
                sub_result[sheet._id] = []
              }
    ...
    add_neighbors(id1: string, id2: string) {
      if (this.neighbors[id1] == null) {
        this.neighbors[id1] = []
      }
      if (this.neighbors[id2] == null) {
        this.neighbors[id2] = []
      }
    ...
    focus() {
      if (this.current_focus && this.network != null) {
        ...
        this.network.focus(node_id, { scale: 1.0, animation: { duration: 100, easingFunction: 'easeInOutQuad' } })
      }
    },
  },
})
